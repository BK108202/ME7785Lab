...
def display_image(image_list, target_size=(25, 33)):
    """
    Processes the first five images to display their preprocessed grayscale version.
    Since preprocess_image now returns only HOG features, we compute the grayscale
    image (converted and resized) directly for visualization.
    ...
    num_images = min(5, len(image_list))
    for img in image_list[:num_images]:
        # Convert to grayscale and resize (same as in preprocess_image)
